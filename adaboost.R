...
rm(list = ls())
cat("\014")
source('~/R/adaboost/func_adaboost.R')
library(rpart)
#raw <- read.csv("R/adaboost/titanic.csv")
#raw <- read.csv("R/adaboost/train_ckd.csv")

raw <- read.csv("R/adaboost/adult.csv",strip.white=TRUE)
...
#train test Split
ind <- sample(1:nrow(raw), size = 0.9*nrow(raw),replace = F)
train <- raw[ind,]
test  <- raw[-ind,]
...
plot(pred_ada$errTrain, type = 'l', col="red") + lines(pred_ada$errTest,col="blue")
